          ...
          if (Array.isArray(child.material)) {
            child.material.forEach((material) => {
              ...
              if (material.metalness !== undefined) {
                material.metalness = Math.min(material.metalness * 1.2, 0.85);
                material.roughness = Math.max(material.roughness * 0.7, 0.1); // Giảm roughness để màu tươi hơn
              }
        ...
        if (child.isMesh && child.material) {
          if (Array.isArray(child.material)) {
            child.material.forEach((material) => {
              ...
              if (material.metalness !== undefined) {
                material.metalness = Math.min(material.metalness * 0.9, 0.8);
                material.roughness = Math.max(material.roughness * 0.7, 0.15);
              }
              material.needsUpdate = true;
            });
          } else {
...
Vị trí: ${colorConfig.types.join(", ")}`
  ...
  const changeWheel = useCallback(
    (wheelId) => {
      ...
      const wheelModel = wheelModels.find((wheel) => wheel.id === wheelId);
      if (wheelModel) {
        openPopup(
          "Thay đổi bánh xe",
      ...
      loader.load(selectedWheelModel.src, (gltf) => {
        ...
        wheelPositions.forEach((position, index) => {
          ...
          wheelMeshNodes.forEach((node, meshIndex) => {
            ...
            const boundingBox = new THREE.Box3().setFromObject(clonedNode);
  ...
  const changeSteeringWheel = useCallback(
    (steeringWheelId) => {
      ...
      if (steeringModel) {
        openPopup(
          "Thay đổi vô lăng",
          `Bạn đã chọn kiểu vô lăng: ${steeringModel.name || steeringWheelId}
...
Thiết kế: ${steeringModel.design || "Hiện đại"}`
      ...
      loader.load(selectedSteeringWheelModel.src, (gltf) => {
        ...
        modelRef.current.traverse((child) => {
          if (
            (child.isMesh &&
              child.name.toLowerCase().includes("steering-wheel")) ||
        ...
        existingSteeringWheelNodes.forEach((node) => {
          console.log("Removing existing steering wheel node:", node.name);
        ...
        steeringWheelMeshNodes.forEach((node) => {
          ...
          clonedNode.rotation.x = selectedSteeringWheelModel.rotation.x;
          clonedNode.rotation.y = selectedSteeringWheelModel.rotation.y;
          clonedNode.rotation.z = selectedSteeringWheelModel.rotation.z;
          ...
          child.material.forEach((material) => {
            ...
            if (material.metalness !== undefined) {
              if (selectedCarIndex === 0) {
                ...
                material.metalness = Math.min(material.metalness * 0.6, 0.7);
                material.roughness = Math.max(material.roughness * 1.5, 0.2);
              } else {
                // Model 2: điều chỉnh để màu tươi sáng hơn, không bị xám
                material.metalness = Math.min(material.metalness * 1.2, 0.85);
                material.roughness = Math.max(material.roughness * 0.6, 0.1);
              }
            }
